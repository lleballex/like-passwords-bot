...
async def start(message: Message, state: FSMContext):
	...
	if not User.get_or_none(user_id=message.from_user.id):
		...
		await message.answer('Хаюшки! Придумай свою ключ. Это может быть любое слово, '
...
async def save_key(message: Message, state: FSMContext):
	if (await state.get_data())['key'] == message.text:
		...
		await message.answer('Ура! Теперь ты можешь добавить свой первый пароль. '
							 'Главное, не забывай ключ', reply_markup=main_kb)
		await state.finish()
	else:
		await state.reset_data()
		await Registration.key_wait.set()
		await message.answer('Хм, не сходится. Давай начнем сначала, придумай свой ключ')
