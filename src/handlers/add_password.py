...
async def wait(message, state):
	...
	await message.answer('Что будем делать дальше?', reply_markup=await get_kb(state))
...
@dp.message_handler(lambda message: message.text == CMDS['save'], state=AddPassword)
...
@dp.message_handler(lambda message: message.text[2:] == CMDS['source'], state=AddPassword)
...
@dp.message_handler(lambda message: message.text[2:] == CMDS['password'], state=AddPassword)
...
@dp.message_handler(lambda message: message.text[2:] == CMDS['username'], state=AddPassword)
async def set_username(message: Message, state: FSMContext):
	await AddPassword.username_wait.set()
	await message.answer('Я тебя слушаю')


@dp.message_handler(lambda message: message.text[2:] == CMDS['phone'], state=AddPassword)
